...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`posts\` (
  	...
  	\`title\` text,
  	\`slug\` text,
  	\`category\` text DEFAULT '产品思考',
  	\`excerpt\` text,
  	\`cover_image_id\` integer,
  	\`reading_time\` text DEFAULT '6 min read',
  ...
  await db.run(sql`CREATE TABLE \`_posts_v\` (
  	...
  	\`version_title\` text,
  	\`version_slug\` text,
  	\`version_category\` text DEFAULT '产品思考',
  	\`version_excerpt\` text,
  	\`version_cover_image_id\` integer,
  	\`version_reading_time\` text DEFAULT '6 min read',
  ...
  await db.run(sql`CREATE TABLE \`docs_keywords\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`keyword\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`docs_keywords_order_idx\` ON \`docs_keywords\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`docs_keywords_parent_id_idx\` ON \`docs_keywords\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`docs\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text,
  	\`slug\` text,
  	\`section\` text DEFAULT '产品文档',
  ...
  await db.run(sql`CREATE TABLE \`_docs_v_version_keywords\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`keyword\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_docs_v_version_keywords_order_idx\` ON \`_docs_v_version_keywords\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_docs_v_version_keywords_parent_id_idx\` ON \`_docs_v_version_keywords\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`_docs_v\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`parent_id\` integer,
  	\`version_title\` text,
  	\`version_slug\` text,
  	\`version_section\` text DEFAULT '产品文档',
  ...
  await db.run(sql`CREATE TABLE \`site_settings\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`eyebrow\` text DEFAULT 'Tick-Driven · Memory-Injected · Tool-Acting',
  	\`hero_title\` text DEFAULT '不是一问一答，而是持续运行的 AI Agent',
  	\`hero_description\` text DEFAULT 'BaiLongma 围绕“持续意识流”构建。它会在 TICK 驱动下继续运行，在有消息时优先响应，在空闲时继续基于任务、记忆与上下文推进下一步动作。现在，这套能力也被整理进一个可部署的官网、博客、文档与后台系统里。',
  	\`primary_label\` text DEFAULT '进入文档中心',
  	\`primary_href\` text DEFAULT '/docs',
  	\`secondary_label\` text DEFAULT '阅读最新博客',
  	\`secondary_href\` text DEFAULT '/blog',
  	\`updated_at\` text,
  	\`created_at\` text
  );
  `)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`docs_keywords\`;`)
  await db.run(sql`DROP TABLE \`docs\`;`)
  await db.run(sql`DROP TABLE \`_docs_v_version_keywords\`;`)
